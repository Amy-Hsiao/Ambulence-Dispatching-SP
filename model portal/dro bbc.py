"""
dro bbc.py — SP + MCVaR + DRO 的 Branch-and-Benders-Cut 入口（plan/09）。

模型切換（隨時可切，互不影響）：
    純 SP：            python "model portal/benders bbc.py"
    SP + MCVaR：       python "model portal/mcvar bbc.py"
    SP + MCVaR + DRO： python "model portal/dro bbc.py"
        ambiguity set 由下方 AMBIGUITY_SET 常數或 config.DRO_AMBIGUITY_SET
        指定（"box" / "ellipsoidal" / "polyhedral"），
        或程式呼叫 run_dro_model(ambiguity_set=..., alpha=..., lam=..., scope=...)。

介面與 `benders bbc.py` 相同：run_dro_model(...) -> (model, summary)。
master 依 Thesis Thm A.1–A.3 對偶重構（box/poly = MILP、ellipsoidal = MISOCP），
oracle、cut、root seeding、EV warm start、parallel oracles 全部沿用。

engine 參數："bbc"（預設）或 "classic"（古典迭代 L-shaped；
DRO-E 的 MISOCP master 若在特定 Gurobi 版本 callback 不穩定時的備援）。

輸出契約：
* RESULT SUMMARY 欄位標籤與 benders bbc 完全一致；VSS/EVPI 印 NA；
  加印 RISK MEASURE (DRO) 區塊與 B&BC 統計區塊。
* log 檔名以 DRO_{BOX|ELL|POLY}_BBC_ 開頭，尾端含 _a{alpha}_l{lambda}_e{scope}。
"""
...
# 預設 ambiguity set（None = 讀 config.DRO_AMBIGUITY_SET）
AMBIGUITY_SET = None

_SET_ALIASES = {
    "box": "dro_box",
    "ellipsoidal": "dro_ellipsoidal",
    "polyhedral": "dro_polyhedral",
    "dro_box": "dro_box",
    "dro_ellipsoidal": "dro_ellipsoidal",
    "dro_polyhedral": "dro_polyhedral",
}
_SET_TAGS = {"dro_box": "BOX", "dro_ellipsoidal": "ELL", "dro_polyhedral": "POLY"}
...
def _resolve_risk_type(ambiguity_set) -> str:
    raw = ambiguity_set
    if raw is None:
        raw = AMBIGUITY_SET
    if raw is None:
        raw = getattr(config, "DRO_AMBIGUITY_SET", "box")
    key = str(raw).strip().lower()
    if key not in _SET_ALIASES:
        raise ValueError(
            f"Unknown ambiguity set {raw!r}; expected box / ellipsoidal / polyhedral."
        )
    return _SET_ALIASES[key]


def _build_dro_log_path(risk_type, scenario_size, sample_ratio, time_limit,
                        mip_gap, alpha, lam, scope) -> Path:
    used = "ALL" if scenario_size is None else scenario_size
    base_name = (
        f"DRO_{_SET_TAGS[risk_type]}_BBC_seed{config.MASTER_SEED}"
        ...
        f"_e{logging_utils._format_value(scope)}"
    )
    return logging_utils._unique_log_path(base_name)


def run_dro_model(ambiguity_set=None, scenario_size=None, sample_ratio=None,
                  time_limit=None, mip_gap=None, alpha=None, lam=None,
                  scope=None, engine="bbc", compute_kpis=True):
    """scope 依 set 對應 ε̄_B / a_E / a_P（None = 讀 config.DRO_*）。"""
    ...
    risk_type = _resolve_risk_type(ambiguity_set)
    scope_kw = {
        "dro_box": "epsilon_box",
        "dro_ellipsoidal": "a_e",
        "dro_polyhedral": "a_p",
    ...
    risk_cfg = risk_core.make_risk_cfg(
        risk_type, alpha=alpha, lam=lam, **({scope_kw: scope} if scope is not None else {})
    )

    log_path = _build_dro_log_path(
        risk_type, scenario_size, sample_ratio, time_limit, mip_gap,
        risk_cfg["alpha"], risk_cfg["lambda"], risk_core.risk_scope(risk_cfg),
    )
    with logging_utils.tee_output(log_path):
        return _run(scenario_size, sample_ratio, time_limit, mip_gap,
                    risk_cfg, engine, compute_kpis)


def _run(scenario_size, sample_ratio, time_limit, mip_gap, risk_cfg, engine,
         compute_kpis):
    ...
    logging_utils.print_run_metadata(
        f"DRO-BBC ({risk_cfg['type']}, worst-case mean-CVaR, engine={engine})",
        instance,
        (
    ...
    print("=" * 50)
    print(f"\nOptimizing SP+MCVaR+DRO Model ({risk_cfg['type']}, "
          f"alpha={risk_cfg['alpha']}, lambda={risk_cfg['lambda']}, "
          f"scope={scope}) — {engine} engine...\n")
    result = lshaped_core.solve(
        instance, S_selected,
        time_limit=time_limit, mip_gap=mip_gap,
        method=engine if engine in ("bbc", "classic") else "bbc",
        risk_cfg=risk_cfg,
    )
    ...
    # ---- risk 摘要（名目 MCVaR + worst-case MCVaR + worst-case p）----------
    ...
    summary = {
        ...
        "risk": {k: v for k, v in risk_stats.items() if k != "worst_p"},
        "worst_p": risk_stats.get("worst_p"),
    ...
    # ---- KPI（重用 benders bbc 的 _evaluate_kpis）--------------------------
    kpis = None
    if compute_kpis:
    ...
    print(" ")
    print(f" - Engine: multi-cut Branch-and-Benders-Cut (DRO {risk_cfg['type']})")
    ...
    print(f" - EVPI(%)   = NA")
    print("-" * 50)
    print(" - RISK MEASURE (worst-case mean-CVaR over ambiguity set):")
    print(f" - ambiguity_set:       {risk_cfg['type']}")
    print(f" - ambiguity_scope:     {scope}")
    print(f" - alpha:               {risk_cfg['alpha']}")
    print(f" - lambda:              {risk_cfg['lambda']}")
    print(f" - first_stage_cost:    {first_stage_cost:.2f}")
    print(f" - expected_Q (p0):     {risk_stats['expected_Q']:.2f}")
    print(f" - VaR (phi*, p0):      {risk_stats['phi_star_VaR']:.2f}")
    print(f" - CVaR_alpha (p0):     {risk_stats['CVaR']:.2f}")
    print(f" - MCVaR (p0):          {risk_stats['MCVaR']:.2f}")
    print(f" - WMCVaR (worst-case): {risk_stats['WMCVaR']:.2f}")
    print(f" - worst_p_max_dev:     {risk_stats['worst_p_max_dev']:.6f}")
    print("-" * 50)
